...
from model_ood import get_angle_to_rot_from_result, get_rotated_image, detect_objects
...
def test_get_rotated_image():
    ...
    assert rotated_img.shape[:2] != img.shape[:2], "Размеры изображения должны измениться после поворота."
...
def test_detect_objects_with_mock_model():
    ...
    mock_model.return_value = [MagicMock(obb=[MagicMock(xywhr=np.array([[0, 0, 10, 20, np.pi / 6]]))])]
    ...
    try:
        ...
        assert annotated_img is not None, "Аннотированное изображение должно быть создано."
...
@pytest.mark.parametrize("obb_data,expected_angle", [
...
def test_acceptance_detect_objects():
    ...
    try:
        angle_to_rot, annotated_img, rotated_image = detect_objects(img)
        assert isinstance(angle_to_rot, float), "Угол должен быть числом с плавающей точкой."
        assert isinstance(annotated_img, np.ndarray), "Аннотированное изображение должно быть массивом."
        assert isinstance(rotated_image, np.ndarray), "Повернутое изображение должно быть массивом."
